...
describe('@cestyle/core/ActionIcon', () => {
  itIsPolymorphic(ActionIcon, defaultProps);
  itRendersChildren(ActionIcon, defaultProps);
  itSupportsFocusEvents(ActionIcon, defaultProps, '.cestyle-ActionIcon-root');
  itSupportsSystemProps({
    component: ActionIcon,
    props: defaultProps,
    displayName: '@cestyle/core/ActionIcon',
  ...
  it('replaces icon with Loader when loading is set to true', () => {
    ...
    expect(notLoading.querySelectorAll('.cestyle-ActionIcon-loading')).toHaveLength(0);

    expect(loading.querySelectorAll('.test-icon')).toHaveLength(0);
    expect(loading.querySelectorAll('.cestyle-ActionIcon-loading')).toHaveLength(1);
  });
});
