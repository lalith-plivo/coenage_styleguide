...
describe('@cestyle/core/Modal', () => {
  ...
  it('allows to hide close button', () => {
    ...
    expect(withButton.querySelectorAll('.cestyle-Modal-close')).toHaveLength(1);
    expect(withoutButton.querySelectorAll('.cestyle-Modal-close')).toHaveLength(0);
  });
  ...
  it('does not render modal header if close button is hidden and title is not passed', () => {
    ...
    expect(withHeader.querySelectorAll('.cestyle-Modal-header')).toHaveLength(1);
    expect(withoutHeader.querySelectorAll('.cestyle-Modal-header')).toHaveLength(0);
  });

  it('has correct displayName', () => {
    expect(Modal.displayName).toStrictEqual('@cestyle/core/Modal');
  });
});
