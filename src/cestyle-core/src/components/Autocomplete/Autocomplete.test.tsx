...
const queries = {
  getDropdown: (container: HTMLElement) =>
    container.querySelector('.cestyle-Autocomplete-dropdown'),
  getItems: (container: HTMLElement) => container.querySelectorAll('.cestyle-Autocomplete-item'),
};

describe('@cestyle/core/Autocomplete', () => {
  checkAccessibility([<Autocomplete {...defaultProps} />]);
  itSupportsInputProps(Autocomplete, defaultProps, 'Autocomplete');
  itSupportsFocusEvents(Autocomplete, defaultProps, '.cestyle-Autocomplete-input');
  itSupportsSystemProps({
    component: Autocomplete,
    props: defaultProps,
    displayName: '@cestyle/core/Autocomplete',
