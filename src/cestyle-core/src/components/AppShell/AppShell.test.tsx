...
describe('@cestyle/core/AppShell', () => {
  itRendersChildren(AppShell, defaultProps);
  itSupportsSystemProps({
    component: AppShell,
    props: defaultProps,
    displayName: '@cestyle/core/AppShell',
  ...
  it('sets fixed position and z-index on Header and Navbar components based on props', () => {
    ...
    expect(_static.querySelector('.cestyle-Header-root')).toHaveStyle({ position: 'static' });
    expect(fixed.querySelector('.cestyle-Header-root')).toHaveStyle({
      zIndex: 476,
      position: 'fixed',
    });

    expect(_static.querySelector('.cestyle-Navbar-root')).toHaveStyle({ position: 'static' });
    expect(fixed.querySelector('.cestyle-Navbar-root')).toHaveStyle({
      zIndex: 476,
      position: 'fixed',
    });
  });
