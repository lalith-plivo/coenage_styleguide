...
describe('@cestyle/core/Menu', () => {
  checkAccessibility([
    <Menu opened {...defaultProps} />,
    <Menu opened={false} {...defaultProps} />,
  ]);
  itSupportsFocusEvents(Menu, defaultProps, '.cestyle-ActionIcon-root');
  itSupportsSystemProps({
    component: Menu,
    props: defaultProps,
    displayName: '@cestyle/core/Menu',
  ...
  it('opens menu when controlled is clicked and trigger prop is "click"', async () => {
    ...
    expect(container.querySelector('.cestyle-Menu-body')).toBe(null);
    await actAsync(() => userEvent.click(screen.getByLabelText('test-menu')));
    expect(container.querySelector('.cestyle-Menu-body')).toBeInTheDocument();
  });
