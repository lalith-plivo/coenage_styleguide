...
describe('@cestyle/core/Slider/Marks', () => {
  it('renders correct marks labels', () => {
    const { container } = render(<Marks {...defaultProps} />);
    const labels = container.querySelectorAll('.cestyle-Slider-markLabel');
    expect(container.querySelectorAll('.cestyle-Slider-markWrapper')).toHaveLength(2);
  ...
  it('calls onChange with mark value when mark label is pressed', () => {
    ...
    userEvent.click(container.querySelectorAll('.cestyle-Slider-markLabel')[0]);
    expect(spy).toHaveBeenLastCalledWith(50);
    userEvent.click(container.querySelectorAll('.cestyle-Slider-markLabel')[1]);
  ...
  it('sets mark left property based on mark value', () => {
    ...
    const marks = container.querySelectorAll('.cestyle-Slider-markWrapper');
    expect(marks[0]).toHaveStyle({ left: '25%' });
    expect(marks[1]).toHaveStyle({ left: '50%' });
  });

  it('has correct displayName', () => {
    expect(Marks.displayName).toStrictEqual('@cestyle/core/SliderMarks');
  });
});
