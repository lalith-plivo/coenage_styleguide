...
import { itSupportsRef, checkAccessibility } from '@cestyle/tests';
...
describe('@cestyle/core/Thumb', () => {
  ...
  it('shows label based on dragging and labelAlwaysOn props', () => {
    ...
    expect(labelAlwaysOn.querySelectorAll('.cestyle-Slider-label')).toHaveLength(1);
    expect(dragging.querySelectorAll('.cestyle-Slider-label')).toHaveLength(1);
    expect(labelOff.querySelectorAll('.cestyle-Slider-label')).toHaveLength(0);
  });

  it('has correct displayName', () => {
    expect(Thumb.displayName).toStrictEqual('@cestyle/core/SliderThumb');
  });
});
