...
describe('@cestyle/core/RadioGroup', () => {
  itSupportsInputWrapperProps(RadioGroup, defaultProps, 'RadioGroup');
  itSupportsSystemProps({
    component: RadioGroup,
    props: defaultProps,
    displayName: '@cestyle/core/RadioGroup',
    excludeOthers: true,
  ...
  itFiltersChildren(RadioGroup, defaultProps, '.cestyle-RadioGroup-radio', [
    <Radio value="test-value-1" label="test-label-1" />,
    <Radio value="test-value-2" label="test-label-2" />,
  ]);
  ...
  it('passes correct name to Radio components', () => {
    ...
    expect(
      withoutName.querySelector('input[type="radio"]').getAttribute('name').includes('cestyle-')
    ).toBe(true);
  ...
  it('has an accessible label', () => {
    const radioGroup = render(
      <RadioGroup label="Select a Framework">
        <Radio value="cestyle" label="CeStyle" />
