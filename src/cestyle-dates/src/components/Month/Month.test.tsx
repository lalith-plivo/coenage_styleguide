...
describe('@cestyle/dates/Month', () => {
  itSupportsSystemProps({
    component: Month,
    props: defaultProps,
    displayName: '@cestyle/dates/Month',
    refType: HTMLTableElement,
  });

  it('renders correct amount of weekdays', () => {
    const { container } = render(<Month month={new Date()} />);
    expect(container.querySelectorAll('.cestyle-Month-weekdayCell')).toHaveLength(7);
  });
  ...
  it('adds styles when month days are within range', () => {
    ...
    expect(withRange.querySelectorAll('.cestyle-Month-firstInRange')).toHaveLength(1);
    expect(withRange.querySelectorAll('.cestyle-Month-lastInRange')).toHaveLength(1);
    expect(withRange.querySelectorAll('.cestyle-Month-inRange')).toHaveLength(11);

    expect(withoutRange.querySelectorAll('.cestyle-Month-firstInRange')).toHaveLength(0);
    expect(withoutRange.querySelectorAll('.cestyle-Month-lastInRange')).toHaveLength(0);
    expect(withoutRange.querySelectorAll('.cestyle-Month-inRange')).toHaveLength(0);
  });
  ...
  it('does not add styles when month days outside of range', () => {
    ...
    expect(container.querySelectorAll('.cestyle-Month-firstInRange')).toHaveLength(0);
    expect(container.querySelectorAll('.cestyle-Month-lastInRange')).toHaveLength(0);
    expect(container.querySelectorAll('.cestyle-Month-inRange')).toHaveLength(0);
  });
  ...
  it('adds partial range styles', () => {
    ...
    expect(container.querySelectorAll('.cestyle-Month-firstInRange')).toHaveLength(1);
    expect(container.querySelectorAll('.cestyle-Month-lastInRange')).toHaveLength(0);
    expect(container.querySelectorAll('.cestyle-Month-inRange')).toHaveLength(9);
  });
  ...
  it('displays selected date', () => {
    ...
    expect(days[6]).toHaveClass('cestyle-Month-selected');
    expect(days[7]).not.toHaveClass('cestyle-Month-selected');
    expect(days[5]).not.toHaveClass('cestyle-Month-selected');
  });
  ...
  it('changes first day of week based on prop', () => {
    ...
    expect(sunday.querySelectorAll('.cestyle-Month-weekdayCell')[0].textContent).toBe('Su');
    expect(sunday.querySelectorAll('.cestyle-Month-weekdayCell')[6].textContent).toBe('Sa');

    expect(monday.querySelectorAll('.cestyle-Month-weekdayCell')[0].textContent).toBe('Mo');
    expect(monday.querySelectorAll('.cestyle-Month-weekdayCell')[6].textContent).toBe('Su');
  });
