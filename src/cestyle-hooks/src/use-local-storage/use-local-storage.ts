...
function serializeJSON<T>(value: T) {
  try {
    ...
    throw new Error('@cestyle/hooks use-local-storage: Failed to serialize the value');
  }
}
...
export function useLocalStorage<T = string>({
      ...
      if (val instanceof Function) {
        setValue((current) => {
          ...
          window.dispatchEvent(
            new CustomEvent('cestyle-local-storage', { detail: { key, value: val(current) } })
          );
          return result;
        });
      } else {
        ...
        window.dispatchEvent(
          new CustomEvent('cestyle-local-storage', { detail: { key, value: val } })
        );
        setValue(val);
      }
    },
  ...
  useWindowEvent('cestyle-local-storage', (event) => {
