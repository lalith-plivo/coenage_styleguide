      ...
      if (node) {
        const processNode = (_node: HTMLElement) => {
          ...
          if (focusElement) {
            ...
            console.warn(
              '[@cestyle/hooks/use-focus-trap] Failed to find focusable element within provided node',
              node
            );
          }
        };
        ...
        setTimeout(() => {
          if (node.ownerDocument) {
            ...
            // eslint-disable-next-line no-console
            console.warn('[@cestyle/hooks/use-focus-trap] Ref node is not part of the dom', node);
          }
        });
