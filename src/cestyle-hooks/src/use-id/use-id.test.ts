...
describe('@cestyle/hooks/use-id', () => {
  it('returns id from hook call', () => {
    const hook = renderHook(() => useId('test-id'));
    expect(hook.result.current).toBe('test-id');
  });

  it('returns random id with prefix "cestyle-" when called without arguments', () => {
    const hook = renderHook(() => useId());
    expect(hook.result.current.includes('cestyle-')).toBe(true);
  });
