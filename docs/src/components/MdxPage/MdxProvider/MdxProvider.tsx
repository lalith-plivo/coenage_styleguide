...
import { Code, Text } from '@cestyle/core';
import { Prism } from '@cestyle/prism';
import { Demo } from '@cestyle/demos';
...
export const components = {
  ...
  a: ({ href, children }: { href: string; children: string }) => {
    const replaced = href.replace('https://cestyle.dev', '');

    if (!replaced.startsWith('http') && replaced.trim().length > 0) {
      return <GatsbyLink to={href.replace('https://cestyle.dev', '')}>{children}</GatsbyLink>;
    }
