...
import { Text, Button, useCeStyleTheme } from '@cestyle/core';
...
function Guide({ dependencies, initScript, children, action, actionLink, withDone }: GuideProps) {
  ...
  return (
    <>
      ...
      <Prism language="bash">{`yarn add ${dependencies || '@cestyle/core @cestyle/hooks'}`}</Prism>
      ...
      <Prism language="bash">
        {`npm install ${dependencies || '@cestyle/core @cestyle/hooks'}`}
      </Prism>
      ...
      {withDone && (
        <>
          <Text weight="semibold" mt={30} mb={5} size="lg">
            All done!
          </Text>
...
function ViteGuide({ dependencies }: GuideProps) {
  return (
    <Guide
      dependencies={dependencies}
      initScript="yarn create vite cestyle-vite --template react-ts"
      withDone
    />
  );
}
...
function NextGuide({ dependencies }: GuideProps) {
  return (
    <Guide
      ...
      actionLink="https://github.com/cestyledev/cestyle-next-template"
    >
      {/* <Text weight="semibold" mt={30} mb={5} size="lg">
...
function GatsbyGuide({ dependencies }: GuideProps) {
  return (
    <Guide
      dependencies={`${dependencies} gatsby-plugin-cestyle`}
      initScript="npm init gatsby"
      action="Get started with Gatsby.js GitHub template"
      actionLink="https://github.com/cestyledev/cestyle-gatsby-template"
      withDone
    >
      <Text weight="semibold" mt={30} mb={5} size="lg">
        Add gatsby-plugin-cestyle in your gatsby.config.js file:
      </Text>
      <Prism language="tsx">plugins: [&apos;gatsby-plugin-cestyle&apos;]</Prism>
    </Guide>
  );
}
...
import { injectStylesIntoStaticMarkup } from '@cestyle/ssr';
...
function RemixGuide({ dependencies }: GuideProps) {
  return (
    <Guide
      dependencies={`${dependencies} @cestyle/ssr`}
      initScript="npx create-remix@latest"
      action="View example setup"
      actionLink="https://github.com/remix-run/remix/tree/main/examples/cestyle"
      withDone
    >
      <Text weight="semibold" mt={30} mb={5} size="lg">
