...
import: "import { SpotlightProvider } from '@cestyle/spotlight';"
source: 'cestyle-spotlight/src'
docs: 'others/spotlight.mdx'
installation: '@cestyle/spotlight'
license: MIT
styles: ['SpotlightProvider']
---

import { SpotlightDemos } from '@cestyle/demos';

## Installation

Package depends on `@cestyle/core` and `@cestyle/hooks`.
...
Note that if you are using [CeStyleProvider](/theming/cestyle-provider/), `SpotlightProvider` must be used as its child:
...
You can setup multiple shortcuts, for example, CeStyle documentation uses the following setup:
...
`@cestyle/spotlight` exports several functions which can be used to perform certain actions
...
You can import `SpotlightAction` type from `@cestyle/spotlight` package:
