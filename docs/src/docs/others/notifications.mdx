...
source: 'cestyle-notifications/src'
docs: 'others/notifications.mdx'
pageTitle: 'CeStyle Notifications'
description: 'CeStyle notifications system'
installation: '@cestyle/notifications'
license: MIT
---

import { NotificationDemos, NotificationsDemos } from '@cestyle/demos';
...
```sh
npm install @cestyle/notifications @cestyle/core @cestyle/hooks
```

Install with yarn:

```sh
yarn add @cestyle/notifications @cestyle/core @cestyle/hooks
```
...
`@cestyle/notifications` package exports the following functions:
...
All functions can be imported from `@cestyle/notifications` package and can be used in any part of your application:
...
```tsx
import { showNotification, hideNotification } from '@cestyle/notifications';
