...
import { SliderDemos, CreateStylesDemos } from '@cestyle/demos';

# Learn the basics

This guide will help you get familiar with core CeStyle concepts.
...
CeStyle documentation includes more than 150 pages, to use it efficiently remember 2 keyboard shortcuts:
...
- `@cestyle/hooks` – collection of 30+ react hooks for state and UI management.
- `@cestyle/styles` – [emotion](https://emotion.sh/) based css-in-js library that is used in all CeStyle components. Usually this package is installed automatically and exported from `@cestyle/core` – there is no need to install it separately, [learn more about createStyles here](/theming/create-styles/).
- `@cestyle/core` – core components library – 100+ components, exports everything from `@cestyle/styles`.
- `@cestyle/notifications` – a fully featured [notifications system](/others/notifications/).
- `@cestyle/prism` – [code highlight](/others/prism/) built with [prism-react-renderer](https://github.com/FormidableLabs/prism-react-renderer).
- `@cestyle/rte` – a Quill based [rich text editor](/others/rte/).
- `@cestyle/dropzone` – manages [files drag 'n' drop](/others/dropzone/) of files to an area or the entire screen.
- `@cestyle/ssr` – [server side rendering](/theming/ssr/) utilities.
- `@cestyle/next` – components and ssr utils for [Next.js integration](/theming/next/).
- `gatsby-plugin-cestyle` – [Gatsby plugin](/theming/gatsby/) to setup ssr.
- `@cestyle/eslint-config` – ESLint and Prettier configuration that the CeStyle project uses.

## Theming

CeStyle theming supports changing colors, spacing, box-shadows, font families, font sizes and many other properties.
To configure theme, wrap your app with a [CeStyleProvider](/theming/cestyle-provider/) component:
...
Learn more about [CeStyleProvider](/theming/cestyle-provider/) and [extending theme](/theming/extend-theme/).

## Dark color scheme

All CeStyle components support light and dark color scheme out of the box.
You can edit the details of each color scheme via [CeStyleProvider](/theming/cestyle-provider/):
...
CeStyle is built with a css-in-js library based on [emotion](https://emotion.sh/).
...
All CeStyle components support these props:
...
CeStyle components work with colors defined in [theme.colors](/theming/extend-theme/#default-colors).
...
Most CeStyle components support `size` prop with xs, sm, md, lg and xl values:
...
function App() {
  return (
    <CeStyleProvider theme={{ spacing: { xs: 15, sm: 20, md: 25, lg: 30, xl: 40 } }}>
      <YourApp />
    </CeStyleProvider>
  );
}
...
Later when you use CeStyle components you can reference these values in `spacing` or other similar props or set the spacing in px:
...
Later when you use CeStyle components you can reference these values in `shadow` prop or define your own shadow:
...
Some CeStyle components support changing root element with `component` prop,
...
function Demo() {
  return (
    <>
      {/* with html element */}
      <Text component="a" href="https://cestyle.dev">
        My link text
      </Text>
...
To use CeStyle components with Next.js Link:
...
import { Button, Paper, TextInput } from '@cestyle/core';
...
@cestyle/core package export types to help you build components and styles with TypeScript:

- `CeStyleTheme` – theme interface defined in CeStyleProvider.
- `ColorScheme` – union of `'light' | 'dark'`.
- `CeStyleColor` – union of all default colors, also accepts any string.
- `CeStyleGradient` – gradient interface used in [Button](/core/button/), [ThemeIcon](/core/theme-icon/) and other components.
- `CeStyleShadow` – union of all default shadows.
- `CeStyleSize` – union of `'xs' | 'sm' | 'md' | 'lg' | 'xl'`.
- `CeStyleNumberSize` – union of `CeStyleSize | number`.
...
```tsx
import type { ButtonProps } from '@cestyle/core';
```

### Polymorphic components types

@cestyle/core package exports `PolymorphicRef` and `PolymorphicComponentProps` types to help you build
custom components with ref forwarding:

```tsx
import { forwardRef } from 'react';
import { PolymorphicRef, PolymorphicComponentProps } from '@cestyle/core';
import { Link } from 'react-router-dom';
...
function Demo() {
  return (
    <>
      ...
      <Text<'a'> component="a" href="https://cestyle.dev">
        CeStyle website
      </Text>
