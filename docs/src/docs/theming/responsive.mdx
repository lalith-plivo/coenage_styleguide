...
import { MediaQueryDemos, CreateStylesDemos } from '@cestyle/demos';

# Responsive styles

`@cestyle/core` package exports several components that can help you organize responsive styles without writing CSS.
...
`theme.breakpoints` are used in all responsive CeStyle components. Breakpoints are set in px (rem, em and other values are not supported).
You can configure these values with [CeStyleProvider](/theming/cestyle-provider/):
...
Most of CeStyle components support `size` prop with `xs`, `sm`, `md`, `lg` and `xl` values.
...
All CeStyle components support `sx` prop with which you can add styles (including responsive styles) to root component element:
...
If you want to add styles with `sx` prop to non-CeStyle component, you can wrap it in [Box](/core/box/):
