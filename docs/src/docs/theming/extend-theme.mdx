...
By default, CeStyle uses system fonts. You can change fonts and other text styles for headings, code and all other components:
...
CeStyle uses [open-color](https://yeun.github.io/open-color/) in default theme with some additions.
...
CeStyle supports 5 sizes for spacing, radius and shadows: xs, sm, md, lg, xl.
...
```tsx
import { Button, Code, CeStyleProvider, Title } from '@cestyle/core';
...
CeStyle supports 5 sizes for breakpoints (`xs`, `sm`, `md`, `lg`, `xl`), which are used in [Grid](/core/grid/), [SimpleGrid](/core/simple-grid/) and other components.
...
CeStyle uses Bootstrap's breakpoint values as defaults:
...
```tsx
import { CeStyleProvider, Box } from '@cestyle/core';
...
TypeScript will only autocomplete CeStyle's default colors when accessing the theme. To add your custom colors to the CeStyleColor type, you can use TypeScript module declaration:

```ts
import { Tuple } from '@cestyle/core';

type CustomColors = 'primaryColorName' | 'secondaryColorName';

declare module '@cestyle/core' {
  export interface CeStyleThemeColorsOverride {
    colors: Record<CustomColors, Tuple<string, 10>>;
  }
}

// or if you want to "extend" standard colors
import { Tuple, DefaultCeStyleColor } from '@cestyle/core';

type ExtendedCustomColors = 'primaryColorName' | 'secondaryColorName' | DefaultCeStyleColor;
...
```ts
declare module '@cestyle/core' {
  export interface CeStyleThemeOther {
