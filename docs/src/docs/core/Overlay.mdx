...
props: ['Overlay']
import: "import { Overlay } from '@cestyle/core';"
source: 'cestyle-core/src/components/Overlay/Overlay.tsx'
docs: 'core/Overlay.mdx'
styles: ['Overlay', 'single-node']
---

import { OverlayDemos, CardDemos } from '@cestyle/demos';
...
You can change overlay opacity (from 0 to 1), color (CSS color value, not connected to CeStyle theme) and z-index (number).
...
```tsx
import { Overlay } from '@cestyle/core';
