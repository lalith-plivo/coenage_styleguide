...
props: ['Input']
import: "import { Input } from '@cestyle/core';"
source: 'cestyle-core/src/components/Input/Input.tsx'
docs: 'core/Input.mdx'
styles: ['Input']
---

import { InputDemos } from '@cestyle/demos';
...
and Input component as base for your own custom inputs with CeStyle theme.

<Demo data={InputDemos.configurator} />

## Variants

Input has 3 variants, all of which are available on all CeStyle inputs.
...
If you want to add your own styles to input it's better to start from scratch rather than overriding CeStyle styles,
use special `headless` variant which does not include any CeStyle styles but still supports all other features: icon, right section, etc.:
