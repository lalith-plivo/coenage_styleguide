...
import: "import { useForm } from '@cestyle/form';"
...
`@cestyle/form` package does not depend on any other libraries, you can use it with or without `@cestyle/core` inputs:
...
`setFieldValue` can be used with all `@cestyle/core` inputs, example with [TextInput](/core/text-input/) component:
...
// value – input value
// onChange – input onChange function, works with all CeStyle and regular inputs
...
// onChange – input onChange event, works with CeStyle Checkbox, Switch and regular input[type="checkbox"]
// error – error message if input is invalid
```
