...
import: "import { useQueue } from '@cestyle/hooks';"
docs: 'hooks/use-queue.mdx'
source: 'cestyle-hooks/src/use-queue/use-queue.ts'
---

import { NotificationsDemos } from '@cestyle/demos';

## Usage

use-queue limits the amount of data in current state and place the rest of it in a queue.
For example, in [@cestyle/notifications](/others/notifications/) package amount of
...
```tsx
import { useQueue } from '@cestyle/hooks';
...
Example of use-queue hook usage in [CeStyle notifications system](/others/notifications/). By default
