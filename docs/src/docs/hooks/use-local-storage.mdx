...
import: "import { useLocalStorage } from '@cestyle/hooks';"
docs: 'hooks/use-local-storage.mdx'
source: 'cestyle-hooks/src/use-local-storage/use-local-storage.ts'
---
...
```tsx
import { useLocalStorage } from '@cestyle/hooks';
...
CeStyle docs website uses this hook to store color scheme information:

```tsx
import { useLocalStorage } from '@cestyle/hooks';
import { ActionIcon, ColorScheme } from '@cestyle/core';
...
You can test this feature by opening 2 tabs with CeStyle docs side by side and changing color scheme
