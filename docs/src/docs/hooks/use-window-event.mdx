...
import: "import { useWindowEvent } from '@cestyle/hooks';"
docs: 'hooks/use-window-event.mdx'
source: 'cestyle-hooks/src/use-window-event/use-window-event.ts'
---
...
import { useWindowEvent } from '@cestyle/hooks';
...
Search focus with `⌘ + K` on mac or `Ctrl + K` on windows and linux on CeStyle docs website:
